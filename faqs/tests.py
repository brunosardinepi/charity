...
class FAQTest(TestCase):
    ...
    def test_faq_page(self):
        ...
        self.assertContains(response, '<span id="upvotes-{}">{}</span>'.format(self.faq.pk, self.faq.upvotes()))
        self.assertContains(response, '<span id="downvotes-{}">{}</span>'.format(self.faq.pk, self.faq.downvotes()))
        self.assertContains(response, self.faq2.question)
        self.assertContains(response, self.faq2.answer)
        self.assertContains(response, '<span id="upvotes-{}">{}</span>'.format(self.faq2.pk, self.faq2.upvotes()))
        self.assertContains(response, '<span id="downvotes-{}">{}</span>'.format(self.faq2.pk, self.faq2.downvotes()))
        self.assertNotContains(response, '<span class="user-vote"')
        ...
        self.assertContains(response, '<span id="upvotes-{}">{}</span>'.format(self.faq2.pk, self.faq2.upvotes()))
        self.assertContains(response, '<span id="downvotes-{}">{}</span>'.format(self.faq2.pk, self.faq2.downvotes()))
        self.assertContains(response, '<span class="user-vote"')
