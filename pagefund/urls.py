...
urlpatterns = [
    ...
    url(r'^campaign/(?P<campaign_pk>\d+)/vote/edit/$', login_required(CampaignViews.CampaignVote.as_view()), name='campaign_edit_vote'),
...
#    url(r'^(?P<page_slug>[\w-]+)/(?P<campaign_pk>\d+)/(?P<campaign_slug>[\w-]+)/edit/vote/$',login_required( CampaignViews.CampaignEditVote.as_view()), name='campaign_edit_vote'),
